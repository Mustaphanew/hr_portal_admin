...
/// Static holder for the root navigator key.
/// Set this from your app_router.dart or main.dart so the notification
/// controller can navigate when the user taps an action.
class AdminNavigator {
  static GlobalKey<NavigatorState>? navigatorKey;
}

class AwesomeNotificationService {
  static const String channelKey = 'admin_alerts_channel';
  static const String channelGroupKey = 'admin_alerts_group';
    ...
    await AwesomeNotifications().initialize(
      ...
      [
        NotificationChannel(
          ...
          channelDescription: 'Channel for admin app alerts',
      ...
      channelGroups: [
        NotificationChannelGroup(
          channelGroupKey: channelGroupKey,
          channelGroupName: 'Admin Alerts Group',
        ),
      ],
      debug: true,
    );
...
class AwesomeNotificationController {
  ...
  static Future<void> onActionReceivedMethod(ReceivedAction action) async {
    ...
    print(
      '[ACTION] key=${key.isEmpty ? '(body)' : key} type=${action.actionType}',
    );
    ...
    if (isBodyTap || key == 'OPEN') {
      ...
      if (route != null && route.isNotEmpty) {
        final ctx = AdminNavigator.navigatorKey?.currentContext;
